...
const PACKAGE_SIZES = [
  ...
  { id: 'medium', label: 'Medium', desc: 'Backpack', icon: '🎒' },
  { id: 'large', label: 'Large', desc: 'Large box', icon: '📦' },
];

function buildDeliveryMapHTML(pickup) {
...
<head>
  ...
  <style>
    ...
    #route-info .route-value { font-size: 16px; font-weight: 700; color: #1c1b1b; }
    #route-info .route-label { font-size: 11px; color: #4b4734; text-transform: uppercase; letter-spacing: 0.5px; }
    #route-info .route-divider { width: 1px; height: 30px; background: #cec6ad; }
  </style>
</head>
<body>
  <div id="map"></div>
  ...
  <script>
    ...
    var routePolyline = null, dropoffMarker = null;
      ...
      fetch(url)
        ...
        .then(function(data) {
          if (data.code === 'Ok' && data.routes.length > 0) {
            ...
            routePolyline = L.polyline(coords, { color: '#6d5e00', weight: 4, opacity: 0.9 }).addTo(map);
        ...
        .catch(function() {
          ...
          var a = Math.sin(dLat/2)*Math.sin(dLat/2) + Math.cos(${pLat}*Math.PI/180)*Math.cos(lat*Math.PI/180)*Math.sin(dLng/2)*Math.sin(dLng/2);
          var dist = R * 2 * Math.atan2(Math.sqrt(a), Math.sqrt(1-a));
          routeDistanceM = dist * 1000;
          routePolyline = L.polyline([[${pLat}, ${pLng}], [lat, lng]], { color: '#6d5e00', weight: 3, dashArray: '8, 8' }).addTo(map);
          ...
          document.getElementById('route-fare').textContent = 'UGX ' + Math.ceil(1500 + dist*700).toLocaleString();
          document.getElementById('route-info').style.display = 'flex';
          map.fitBounds(routePolyline.getBounds(), { padding: [40, 40] });
...
export default function DeliveryDetailsScreen({ route, navigation }) {
  ...
  useEffect(() => {
    if (roadDistanceM > 0) {
      setFareEstimate(Math.ceil(1500 + (roadDistanceM / 1000) * 700));
    }
  }, [roadDistanceM]);
  ...
  const handleConfirm = async () => {
    if (!pickupCoords || !dropoffCoords || !recipientName) return;
    setLoading(true);
    try {
      const { data } = await bookingAPI.create({
        ...
        item_description: `Package delivery (${packageSize})`,
        recipient_name: recipientName,
        recipient_phone: `256${recipientPhone}`,
      ...
      const msg = err.response?.data?.error || 'Could not create delivery. Please try again.';
      if (activeId) {
        showModal({
          icon: '⚠️',
          ...
          actions: [
            ...
            { label: 'Cancel It', primary: true, onPress: async () => {
              try {
                ...
                showModal({ icon: '✅', title: 'Cancelled', message: 'Your active booking has been cancelled. You can now create a new delivery.' });
              } catch (e) {
      ...
      <View style={styles.container}>
        ...
        <View style={[styles.bottomSheet, searchFocused && styles.bottomSheetExpanded]}>
          ...
          <ScrollView style={styles.sheetScroll} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
                ...
                <View style={styles.searchResults}>
                  {searching ? (
                    ...
                    searchResults.map((result, index) => (
                      <TouchableOpacity key={index} style={styles.searchResultItem} onPress={() => selectLocation(result)} activeOpacity={0.7}>
            ...
            <View style={styles.packageRow}>
              {PACKAGE_SIZES.map((size) => (
                ...
                >
                  ...
                  <Text style={[styles.packageLabel, packageSize === size.id && styles.packageLabelActive]}>{size.label}</Text>
                  <Text style={styles.packageDesc}>{size.desc}</Text>
                </TouchableOpacity>
              ))}
            </View>

            <Text style={[styles.sectionTitle, { marginTop: spacing.xl }]}>Recipient</Text>
            ...
            <Text style={[styles.sectionTitle, { marginTop: spacing.xl }]}>Payment</Text>
            {paymentMethods.length === 0 ? (
              ...
              <TouchableOpacity
                ...
                onPress={() => {
                  const idx = paymentMethods.findIndex(m => m.id === selectedPayment?.id);
                  setSelectedPayment(paymentMethods[(idx + 1) % paymentMethods.length]);
                }}
                activeOpacity={0.7}
              >
          ...
          <View style={styles.confirmWrapper}>
            <TouchableOpacity
              style={[styles.confirmButton, (!pickupCoords || !dropoffCoords || !recipientName || loading) && styles.buttonDisabled]}
              onPress={handleConfirm}
              disabled={!pickupCoords || !dropoffCoords || !recipientName || loading}
              activeOpacity={0.8}
            >
              {loading ? (
...
const styles = StyleSheet.create({
  ...
  dashedLine: { position: 'absolute', left: 28, top: 42, bottom: 42, width: 2, borderStyle: 'dashed', borderWidth: 1, borderColor: colors.outlineVariant },
  addressRow: { flexDirection: 'row', alignItems: 'center', gap: spacing.md, padding: spacing.sm },
  pickupDot: { width: 12, height: 12, borderRadius: 6, backgroundColor: '#22c55e' },
  dropoffDot: { width: 12, height: 12, borderRadius: 6, backgroundColor: colors.error },
  addressInputWrapper: { flex: 1, backgroundColor: colors.surface, borderRadius: radius.md, padding: spacing.sm + 2, borderWidth: 1, borderColor: colors.outlineVariant },
  dropoffInput: { borderWidth: 2, borderColor: colors.primaryContainer },
  addressLabel: { ...typography.labelSm, color: colors.secondary, textTransform: 'uppercase', letterSpacing: 1 },
  addressValue: { ...typography.titleMd, color: colors.onSurface, marginTop: 2 },
  addressTextInput: { ...typography.titleMd, color: colors.onSurface, padding: 0, marginTop: 2 },
  searchResults: { backgroundColor: colors.surface, borderRadius: radius.xl, marginTop: spacing.sm, borderWidth: 1, borderColor: colors.outlineVariant, overflow: 'hidden' },
  searchResultItem: { flexDirection: 'row', alignItems: 'center', padding: spacing.md, gap: spacing.sm, borderBottomWidth: 1, borderBottomColor: colors.surfaceContainer },
  searchResultIcon: { fontSize: 16 },
  searchResultText: { flex: 1, ...typography.bodyMd, color: colors.onSurface },
  noResults: { padding: spacing.lg, ...typography.bodyMd, color: colors.onSurfaceVariant, textAlign: 'center' },
  sectionTitle: { ...typography.titleLg, color: colors.onSurface, marginBottom: spacing.md },
  packageRow: { flexDirection: 'row', gap: spacing.md, marginBottom: spacing.lg },
  packageCard: { flex: 1, padding: spacing.md, borderRadius: 16, borderWidth: 2, borderColor: colors.outlineVariant, backgroundColor: colors.surface, alignItems: 'center' },
  packageCardActive: { borderColor: colors.onSurface, backgroundColor: colors.primaryContainer },
  packageIcon: { fontSize: 24, marginBottom: spacing.sm },
  packageLabel: { ...typography.labelLg, color: colors.onSurface },
  packageLabelActive: { fontWeight: '700' },
  packageDesc: { ...typography.labelSm, color: colors.onSurfaceVariant, textAlign: 'center', marginTop: 2 },
  recipientCard: { marginBottom: spacing.xl },
  ...
  phonePrefix: {
    width: 72, height: 56, borderRadius: radius.xl,
    borderWidth: 1, borderColor: colors.outlineVariant,
    backgroundColor: colors.surfaceContainerLow, alignItems: 'center', justifyContent: 'center',
  },
  phonePrefixText: { ...typography.bodyMd, color: colors.onSurface },
  phoneInput: { flex: 1 },
  confirmWrapper: {
    paddingHorizontal: spacing.lg, paddingBottom: Platform.OS === 'ios' ? 28 : 16, paddingTop: spacing.sm, backgroundColor: colors.surface,
  },
  confirmButton: {
  ...
  confirmButtonText: { ...typography.headlineMd, color: colors.onPrimaryContainer },
  savedPlacesSection: {
  ...
  savedPlaceIcon: { fontSize: 22 },
  savedPlaceInfo: { flex: 1 },
  savedPlaceLabel: { ...typography.titleMd, color: colors.onSurface },
  savedPlaceAddress: { ...typography.bodySm, color: colors.onSurfaceVariant, marginTop: 2 },
  savedPlaceArrow: { fontSize: 18, color: colors.primary, fontWeight: '600' },
  momoBadge: {
    flexDirection: 'row',
    alignItems: 'center',
    padding: spacing.lg,
  ...
  momoIcon: { width: 40, height: 40, borderRadius: 20, backgroundColor: '#ffcc00', alignItems: 'center', justifyContent: 'center', marginRight: spacing.md },
  momoIconText: { fontSize: 10, fontWeight: '800', color: '#713f12' },
  momoInfo: { flex: 1 },
  momoName: { ...typography.titleMd, color: colors.onSurface },
  momoNumber: { ...typography.bodySm, color: colors.onSurfaceVariant, marginTop: 2 },
  momoChevron: { fontSize: 18, color: colors.onSurfaceVariant, fontWeight: '600' },
});
