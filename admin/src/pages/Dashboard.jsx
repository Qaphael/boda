...
export default function Dashboard() {
  ...
  const loadDashboard = async () => {
    ...
    try {
      ...
      setRecentRiders((ridersRes.data.riders || []).slice(0, 5));
      setTicketStats(ticketsRes.data.stats || {});
    } catch (err) {
  ...
  const statCards = [
    { label: 'Total Riders', value: stats.riders?.total || 0, sub: null, color: 'text-on-surface' },
    { label: 'Online Riders', value: stats.riders?.online || 0, sub: null, color: 'text-primary', bar: true },
    { label: 'Pending Verif.', value: stats.riders?.pending || 0, sub: stats.riders?.pending > 0 ? 'Immediate action' : 'All clear', color: 'text-tertiary' },
    { label: 'Total Bookings', value: stats.bookings?.total || 0, sub: 'All time', color: 'text-on-surface' },
    { label: 'Active Bookings', value: stats.bookings?.active || 0, sub: null, color: 'text-on-surface', pulse: stats.bookings?.active > 0 },
    { label: "Today's Bookings", value: stats.bookings?.today || 0, sub: null, color: 'text-on-surface' },
    { label: 'Total Users', value: stats.users?.total || 0, sub: null, color: 'text-on-surface' },
    { label: 'Held Payments', value: stats.payments?.held || 0, sub: stats.payments?.held > 0 ? 'Flagged risk' : 'None', color: 'text-error' },
  ];

  const quickActions = [
    {
      title: `${stats?.riders?.pending || 0} Pending Rider Applications`,
      desc: 'Awaiting admin verification and approval.',
      badge: stats?.riders?.pending > 0 ? 'URGENT' : null,
      badgeClass: 'bg-tertiary-fixed text-on-tertiary-fixed',
      icon: 'person_add',
      hoverBg: 'hover:bg-primary-container hover:border-primary',
      link: '/riders',
    },
    {
      title: `${stats?.payments?.held || 0} Held Payments`,
      desc: 'Payments held in escrow awaiting release.',
      badge: stats?.payments?.held > 0 ? 'FLAGGED' : null,
      badgeClass: 'bg-error-container text-error',
      icon: 'money_off',
      hoverBg: 'hover:bg-error-container hover:border-error',
      link: '/payments',
    },
    {
      title: `${stats?.bookings?.active || 0} Active Bookings`,
    ...
    {
      title: `${ticketStats?.open_count || 0} Open Support Tickets`,
      desc: ticketStats?.urgent > 0 ? `${ticketStats.urgent} urgent tickets` : 'No urgent tickets',
      badge: ticketStats?.urgent > 0 ? 'URGENT' : null,
      badgeClass: 'bg-error-container text-error',
      icon: 'support_agent',
      hoverBg: 'hover:border-primary',
      link: '/support',
    ...
    <div className="space-y-6 p-3 sm:p-4 lg:p-6">
        ...
        {quickActions.map((action) => (
          ...
          >
            ...
            {!action.value && !action.desc && (
              <p className="text-body-sm text-on-surface-variant mt-3">{ticketStats?.open_count || 0} tickets open</p>
            )}
      ...
      <div className="border border-outline-variant rounded bg-surface overflow-hidden">
        <div className="p-4 border-b border-outline-variant flex items-center justify-between">
          <h3 className="text-headline-sm">Recent Rider Applications</h3>
        ...
        <div className="overflow-x-auto">
          {recentRiders.length === 0 ? (
            <div className="p-8 text-center text-on-surface-variant text-body-sm">No pending rider applications</div>
          ) : (
            <table className="w-full text-left border-collapse">
              <thead className="bg-surface-container-low">
                <tr>
                  <th className="px-4 py-3 text-[11px] uppercase tracking-widest text-outline font-medium">Rider</th>
                  <th className="px-4 py-3 text-[11px] uppercase tracking-widest text-outline font-medium">Phone</th>
                  <th className="px-4 py-3 text-[11px] uppercase tracking-widest text-outline font-medium">Plate Number</th>
                  <th className="px-4 py-3 text-[11px] uppercase tracking-widest text-outline font-medium">Applied</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-outline-variant/30">
                {recentRiders.map((rider) => (
                  <tr key={rider.id} className="hover:bg-surface-container-low transition-colors cursor-pointer" onClick={() => navigate('/riders')}>
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-3">
                        <div className="w-8 h-8 rounded-full bg-primary/10 flex items-center justify-center text-primary font-bold text-xs">{rider.name?.split(' ').map(n => n[0]).join('').toUpperCase().slice(0, 2)}</div>
                        <div>
                          <p className="text-label-md font-bold">{rider.name}</p>
                          <p className="text-body-sm text-outline">{rider.national_id || 'N/A'}</p>
                        </div>
                      </div>
                    </td>
                    <td className="px-4 py-3 text-body-sm">{rider.phone}</td>
                    <td className="px-4 py-3 font-mono text-body-sm">{rider.plate_number}</td>
                    <td className="px-4 py-3 text-body-sm text-outline">{rider.created_at ? new Date(rider.created_at).toLocaleDateString() : 'N/A'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
